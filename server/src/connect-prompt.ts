import { PYTHON_HPKE_REFERENCE, PYTHON_HPKE_REFERENCE_V3 } from "./python-reference.ts";
...
export const CONNECT_PROMPT_V3 = `# Connect Efferent v3
...
1. Connect the supplied remote MCP URL using Streamable HTTP. Do not add authentication or the key.
   Its tools are only for archive status, sealed-day listings and ciphertext links.
2. Create a private Python virtual environment and install \`pyhpke==0.6.3\`.
3. Save the code below as \`efferent_hpke.py\`. Save the complete four-field handoff as a separate
   owner-only file and set both files to mode 600 on POSIX systems.
4. Use \`archive_status\` and \`list_sealed_days\` to choose the dates needed for the question. Run
...
5. Delete the temporary handoff file when the local session no longer needs it.

No Efferent repository, Deno installation, local MCP server or gateway restart is required. Do not
search for or clone a repository. The script derives the ciphertext URL from the keyless MCP URL;
its requests contain only the bucket id and date.

\`\`\`python
${PYTHON_HPKE_REFERENCE_V3}\`\`\`
